# Export stylometric analysis JSON from stylo classify/rolling RDS outputs, for
# BOTH the content and style axes independently (see run_jams_quadrant.R).
...
analyze_paper_from_stylo <- function(i, classify_result, dist_mat, train_names, train_class,
                                      geo_centroid, alg_centroid, feats_used) {
  ...
  score <- alg_pull - geo_pull
  ord <- order(abs(score), decreasing = TRUE)[1:15]
...
# Analyze one axis (content or style): loads its classify()/rolling.classify()
# RDS outputs, computes corpus health, per-paper nearest-neighbor/discriminator
# analysis, and rolling-slice statistics. `rolling_prefix` matches the label
# used in run_jams_quadrant.R (rolling_<prefix>_<stem>.rds).
analyze_axis <- function(full_rds, excerpt_rds, rolling_prefix) {
  full <- readRDS(file.path(out_dir, full_rds))
  excerpt <- readRDS(file.path(out_dir, excerpt_rds))
  ...
  papers_full <- lapply(seq_len(nrow(full$frequencies.test.set)), function(i) {
    analyze_paper_from_stylo(i, full, distance_table, ref_labels, ref_class, geo_centroid, alg_centroid, feats_used)
  })
  ...
  papers_excerpt <- lapply(seq_len(nrow(excerpt$frequencies.test.set)), function(i) {
    analyze_paper_from_stylo(i, excerpt, dt_ex, ref_labels, ref_class, geo_centroid, alg_centroid, feats_used)
  })
  names(papers_excerpt) <- rownames(excerpt$frequencies.test.set)

  rolling_export <- list()
  for (stem in paper_stems) {
    rds_path <- file.path(out_dir, paste0("rolling_", rolling_prefix, "_", stem, ".rds"))
  ...
  list(
    ...
    excerpts = list(success_rate = as.numeric(excerpt$success.rate), papers = papers_excerpt),
...
content_analysis <- analyze_axis("classify_content_full_papers.rds", "classify_content_excerpts.rds", "content")
style_analysis <- analyze_axis("classify_style_full_papers.rds", "classify_style_excerpts.rds", "style")

stylo_results_path <- file.path(out_dir, "stylo_results.json")
stylo_results <- if (file.exists(stylo_results_path)) fromJSON(stylo_results_path, simplifyVector = FALSE) else list()

confusion_for <- function(axis_name, rolling_export, papers_full, expert_labels) {
  out <- list(rolling = list(), whole_paper = list())
  for (nm in names(expert_labels)) {
    ...
    if (!is.null(roll)) {
      ...
      out$rolling[[nm]] <- list(expert = expert, predicted = pred, match = pred == expert)
    }
    if (nm %in% names(papers_full)) {
      pred <- papers_full[[nm]]$predicted
      out$whole_paper[[nm]] <- list(expert = expert, predicted = pred, match = pred == expert)
    }
  }
  out
}

expert_content_labels <- stylo_results$content$accuracy$expert_labels
expert_style_labels <- stylo_results$style$accuracy$expert_labels
if (is.null(expert_content_labels)) expert_content_labels <- list()
if (is.null(expert_style_labels)) expert_style_labels <- list()

confusion <- list(
  content = confusion_for("content", content_analysis$rolling, content_analysis$full_papers$papers, expert_content_labels),
  style = confusion_for("style", style_analysis$rolling, style_analysis$full_papers$papers, expert_style_labels)
)
...
manifest_meta <- if (file.exists(manifest_path)) {
  ...
  list(version = if (!is.null(m$version)) m$version else NA, n_sources = n_src)
} else list()
...
export <- list(
  meta = list(
    ...
    method = "Burrows Delta via stylo::classify / rolling.classify, two independent axes (content, style)",
    features = sprintf("%d English stopwords (Snowball), used after culling", length(stopwords::stopwords("en", "snowball"))),
    train_dir_content = Sys.getenv("STYLO_CONTENT_TRAIN_DIR", "corpus_classify/reference_research/balanced_content"),
    train_dir_style = Sys.getenv("STYLO_STYLE_TRAIN_DIR", "corpus_classify/reference_research/balanced_style"),
    ...
    independence_note = "content and style are labeled independently per training chunk (scripts/retag_content_style.py); see output/corpus_diagnosis_stylo.json for the content x style independence check."
  ),
  content = content_analysis,
  style = style_analysis,
  content_accuracy = stylo_results$content$accuracy,
  style_accuracy = stylo_results$style$accuracy,
  quadrant = stylo_results$quadrant,
  quadrant_accuracy = stylo_results$quadrant_accuracy,
  confusion = confusion,
  benchmarks = list(
